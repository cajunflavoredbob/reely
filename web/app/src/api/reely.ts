...
export class ReelyClient extends EventTarget {
  ...
  // only covered the OFFLINE queue. Production reely 0.5.20 (Lance on
  // galactica, 2026-05-27 ~02:55 UTC) still tripped the WS 100/10s rate
  ...
  sendMessage(msg: ServerMessage) {
    if (this.ws.readyState !== WebSocket.OPEN) {
      ...
      if (msg.type === "rate") {
        ...
        // Post-mortem from production reely 0.5.9 (Lance1 on galactica,
